...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .btn{width:100%;padding:14px;background:linear-gradient(135deg,var(--accent),#0099cc);border:none;border-radius:8px;color:#000;font-family:'Syne',sans-serif;font-size:15px;font-weight:700;cursor:pointer;transition:.2s;box-shadow:0 4px 20px rgba(0,212,255,.3);}
...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .btnlo{width:100%;padding:7px;border-radius:6px;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);color:#fca5a5;font-size:12px;font-family:'Syne',sans-serif;cursor:pointer;transition:.15s;text-align:center;text-decoration:none;display:block;}
        ...
        .abtn{padding:12px 22px;border-radius:8px;font-family:'Syne',sans-serif;font-size:13px;font-weight:700;cursor:pointer;border:none;transition:.15s;display:inline-flex;align-items:center;gap:8px;}
...
<main class="main">
  ...
  <div class="pipeline">
    ...
    <div class="psteps">
      <div class="pstep"><div class="si">&#x1F441;</div><div class="sn">Monitor</div></div>
...
function runDemo() {
  ...
  fetch('/api/demo-incident', {method: 'POST'})
    .then(function(r) { return r.json(); })
    .then(function(data) {
      btn.textContent = 'Demo CRITICAL Incident';
      btn.disabled = false;
      if (data.error) {
        alert('Error: ' + data.error);
      } else {
